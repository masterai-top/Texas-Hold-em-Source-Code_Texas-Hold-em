...
namespace majong
{
    namespace gameserver
    {
        ...
        int GameServer::onGameSoMessage(const long lPlayerID, const vector<char> &vecMsgData)
        {
            ...
            //校验版本
            if (somsg.nver() != XGameMJProto::iProtoVersion)
            {
                LOG_WARN("client proto version : \"" << somsg.nver() << "\" server proto version : \"" << XGameMJProto::iProtoVersion << "\";");
            }

            //校验命令字
            if (somsg.ncmd() > XGameMJProto::EMsgEnd || somsg.ncmd() < XGameMJProto::EmiBegin)
            {
            ...
            RLOG_DEBUG << "process game so reqeust, msg size = " << vecOutBuffer.size() << ", msg data len: " << somsg.vecmsgdata().length() << ", msgID:" << somsg.ncmd() << endl;
        ...
        int GameServer::doSendGameData(const long &pid, int eMSG, const vector<char> &vecData)
        {
            ...
            sTSoMsg.set_nver(XGameMJProto::iProtoVersion);
